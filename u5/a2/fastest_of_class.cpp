...
std::vector<Spaceship> fastest_of_class(std::vector<Spaceship> fleet)
{
    ...
    {
        if (result.empty() || !(result.back() == fleet[i])) // don't want to add duplicate entries. check for emptyness because "calling back() on an empty container causes undefined behavior"
        {
